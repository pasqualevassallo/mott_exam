...
# ============================================
# ESTRAI L'IPERVOLUME DAL FILENAME
# ============================================

def extract_hypervolume_from_filename(filename):
    """Estrae l'ipervolume dal nome del file .npz"""
    import re
    # Pattern: cerca l'ultimo numero nel filename (es. quantcomm_1_100_6372134.npz)
    match = re.search(r'_(\d+)\.npz
...
def plot_pareto_front(ys, best_solutions, hypervolume, filename='pareto_front.png'):
    ...
    # Disegna tutte le soluzioni (punti blu)
    ax.scatter(f1, f2, s=60, c='#1f77b4', alpha=0.7, edgecolors='none', zorder=2)
    
    # Evidenzia le soluzioni notevoli con diamanti colorati
    markers_info = {
        'Migliore Comunicazione': {'color': '#d62728', 'label': 'Best communication'},  # Rosso
        'Bilanciata': {'color': '#ff7f0e', 'label': 'Balanced'},  # Arancione
        'Minor Costo': {'color': '#2ca02c', 'label': 'Lowest cost'}  # Verde (anche se non visibile nell'esempio)
    }
    
    for name, (x, y) in best_solutions.items():
        info = markers_info[name]
        ax.scatter(y[0], y[1], s=200, c=info['color'], marker='D', 
                  edgecolors='black', linewidth=1.5, label=info['label'], zorder=5)
    
    # Stile del grafico
    ax.set_xlabel('Cost of communications between rovers and motherships — $J_1
...
if __name__ == '__main__':
    ...
    try:
        ...
        # ESTRAI IPERVOLUME DAL FILENAME
        hypervolume = extract_hypervolume_from_filename(filename)
        if hypervolume is not None:
            print(f"\n{'='*70}")
            print(f"IPERVOLUME (dal filename): {hypervolume:.3f}")
            print(f"{'='*70}")
        else:
            print("\n⚠ Impossibile estrarre l'ipervolume dal filename")
            hypervolume = None
        ...
        print("   - quantcomm_1_100_6372134.npz"), filename)
    if match:
        return float(match.group(1)) / 1e7  # Dividi per 10^7 per normalizzare
    return None
...
if __name__ == '__main__':
    ...
    ax.set_ylabel('Cost of building and operating the\ntwo satellite constellations — $J_2
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("   - quantcomm_1_100_6372134.npz"), 
                  fontsize=11, family='serif')
    ax.set_title('Objective trade-offs in the solution portfolio', 
                 fontsize=12, fontweight='bold', pad=15, family='serif')
    
    # Griglia sottile
    ax.grid(True, alpha=0.2, linewidth=0.5, color='gray')
    ax.set_axisbelow(True)
    
    # Box con ipervolume (stile del tuo esempio)
    if hypervolume is not None:
        textstr = f'Hypervolume:\n{hypervolume:.3f}'
        props = dict(boxstyle='round', facecolor='white', edgecolor='gray', linewidth=1)
        ax.text(0.98, 0.98, textstr, transform=ax.transAxes, fontsize=10,
                verticalalignment='top', horizontalalignment='right', bbox=props, family='serif')
    
    # Legenda pulita
    ax.legend(loc='upper right', fontsize=9, frameon=True, framealpha=0.95, 
              edgecolor='gray', fancybox=False)
    
    # Limiti degli assi per migliorare la visualizzazione
    ax.set_xlim(f1.min() - 0.05, f1.max() + 0.1)
    ax.set_ylim(f2.min() - 0.05, f2.max() + 0.1)
...
if __name__ == '__main__':
    ...
    # CARICA IL FILE (sostituisci con il tuo file .npz)
    filename = 'quantcomm_639739.npz'  # O il file più recente generato
